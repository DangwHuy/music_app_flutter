...
class _ConversationsScreenState extends State<ConversationsScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    final currentUser = FirebaseAuth.instance.currentUser;
    if (currentUser == null) {
      return Scaffold(appBar: AppBar(title: const Text('Tin nhắn')),
      ...
      builder: (context, userSnapshot) {
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            ...
            title: titleWidget,
            actions: [
              IconButton(icon: const Icon(Icons.psychology_alt_outlined),
                  onPressed: () =>
                      Navigator.of(context).push(MaterialPageRoute(
                          builder: (context) => const AiChatScreen()))),
              IconButton(icon: const Icon(Icons.edit_note_outlined),
                  onPressed: () =>
                      Navigator.of(context).push(MaterialPageRoute(
  ...
  Widget _buildSearchResults() {
    // Giữ nguyên (Avatar đã được đồng bộ ở đây)
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance.collection('users').where(
          'username', isGreaterThanOrEqualTo: _searchQuery).where(
          'username', isLessThanOrEqualTo: '$_searchQuery\uf8ff').snapshots(),
      builder: (context, snapshot) {
        if (!snapshot.hasData)
          return const Center(child: CircularProgressIndicator());
        final users = snapshot.data?.docs.where((doc) =>
        doc.id != FirebaseAuth.instance.currentUser?.uid).toList() ?? [];
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final name = userData['displayName']?.isNotEmpty == true
                ? userData['displayName']
                : userData['username'];
            return ListTile(
              leading: CircleAvatar(radius: 30,
                  backgroundImage: userData['avatarUrl'] != null ? NetworkImage(
      ...
      builder: (context, snapshot) {
        if (snapshot.hasError) {
          return const Center(child: Text(
              'Lỗi: Vui lòng tạo chỉ mục Firestore cho bộ lọc này.',
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final bool isUnread = lastSenderId != null &&
            ...
            if (isGroup) {
              // *** THAY ĐỔI: Load và hiển thị Avatar nhóm ***
              final groupAvatarUrl = convoData['groupAvatarUrl'] as String?;
              return ListTile(
                leading: CircleAvatar(
                  radius: 30,
                  backgroundColor: Colors.grey.shade800,
                  backgroundImage: groupAvatarUrl != null ? NetworkImage(
                      groupAvatarUrl) : null, // Hiển thị ảnh nếu có URL
                  child: groupAvatarUrl == null
                      ? const Icon(Icons.group, color: Colors.grey)
                      : null, // Fallback icon nếu không có
                ),
                title: Text(convoData['groupName'] ?? 'Trò chuyện nhóm',
                    style: const TextStyle(
                        color: Colors.white, fontWeight: FontWeight.bold)),
                subtitle: Text(convoData['lastMessage'] ?? '', maxLines: 1,
                    ...
                    Navigator.of(context).push(MaterialPageRoute(
                        builder: (context) =>
                            ChatScreen(conversationId: convo.id,
                                groupName: convoData['groupName'],
                                isGroup: true))),
              );
            }

            final String otherUserId = participants.firstWhere((id) =>
            id != currentUserId, orElse: () => '');
            if (otherUserId.isEmpty) return const SizedBox.shrink();

            final String? nickname = nicknames[otherUserId];

            return FutureBuilder<DocumentSnapshot>(
              future: nickname == null ? FirebaseFirestore.instance.collection(
                  'users').doc(otherUserId).get() : null,
              builder: (context, userSnapshot) {
                String displayName;
                String? avatarUrl;
                if (nickname != null) {
                  ...
                  final userData = userSnapshot.data!.data() as Map<
                      String,
                      dynamic>? ?? {};
                  displayName = userData['displayName']?.isNotEmpty == true
                      ? userData['displayName']
                      : userData['username'];
                  ...
                  confirmDismiss: (direction) async {
                    if (direction == DismissDirection.endToStart) {
                      final confirmed = await showDialog<bool>(context: context,
                          builder: (context) =>
                              AlertDialog(
                                  ...
                                  content: const Text(
                                      'Hành động này sẽ xóa vĩnh viễn cuộc trò chuyện.'),
                                  actions: [
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                          content: Text(
                              'Tính năng ghim đang được phát triển!')));
                      return false;
                    }
                  },
                  child: ListTile(
                    leading: CircleAvatar(
                      ...
                      child: avatarUrl == null ? const Icon(
                          Icons.person, size: 30, color: Colors.grey) : null,
                    ),
                    title: Text(
                        displayName, style: const TextStyle(color: Colors.white,
                        fontWeight: FontWeight.bold)),
                    subtitle: Text(convoData['lastMessage'] ?? '', maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                        style: TextStyle(color: isUnread ? Colors.white : Colors
                            .grey, fontWeight: isUnread
                            ? FontWeight.bold
                            : FontWeight.normal)),
                    trailing: const Icon(
